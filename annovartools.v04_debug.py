...
class Multianno(object):
    def __init__(self,record,transcript,header=None):
        ...
        Func_refGene={'exonic':self.exonic_func,
                ...
                'UTR3':self.utr_func,
                'UTR5':self.utr_func,
                'intronic':self.intronic_func,
                'intronic\\3xbsplicing':self.intronic_func
    ...
    def intronic_func(self,INFO,transcript):
        ...
        if INFO['GeneDetail.refGene'] in [[''],['.'],list()]:
            for k in INFO['Gene.refGene']:
                self.fields['Transcript.refGene']+=transcript[k]
        #INFO could equal dict of GeneDetail + dbscSNV
        for x in ['dbscSNV_ADA_SCORE','dbscSNV_RF_SCORE']:
            if INFO.get(x,['.']) in [[''],['.'],list()]:
                continue
            else:
                try:
                    float(INFO.get(x)[0])
